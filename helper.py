...
def getLargVal(*inA):
    """ Return from several arrays one array with the respective largest value at each position """
    inputlen = len(inA)
    ...
    if type(inList1[0]) != np.ndarray:
        ...
        # Raster share the same Coordinate System but not the same extent
            ...
            for file in inList1:
                ...
                # Wait for subprocess to finish, or pyhton continues and returns error when output is not there yet
                child.wait()
